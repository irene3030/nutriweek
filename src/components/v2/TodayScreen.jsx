...
import { Plus, MapPin, ShoppingCart, Clock, ChevronRight, ChevronLeft, Cookie, X } from 'lucide-react';
...
const DAY_LABELS = {
  ...
  [0]: 'Hoy', [1]: 'Mañana', [2]: 'Pasado mañana', [3]: 'En 3 días',
};

function getDays(base) {
  return [base - 1, base, base + 1].map(offset => ({
    offset,
    label: DAY_LABELS[offset] ?? (offset < 0 ? `Hace ${-offset} días` : `En ${offset} días`),
  }));
}
...
export default function TodayScreen({ householdId, hasAiAccess, pantryItems = [] }) {
  ...
  const handleConfirmSlot = async (dateStr, slotId) => {
    ...
    const clearedItems = slot.items.map(it => it.pendingPrep ? { ...it, pendingPrep: false } : it);
    await confirmSlot(dateStr, slotId, clearedItems);
